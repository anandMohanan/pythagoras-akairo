...
module.exports = class TickleCommand extends Command {
  ...
  async exec(msg, { person }) {
    try {
      const body = await fetch("https://nekos.best/api/v1/tickle");
      let resp = await body.json();
      let embed = CreateEmbed("info")
        .setImage(resp.url)
        .setTitle(`${msg.author.username} tickles ${person.username}`);
      msg.reply({
        //content: `\>>> ${msg.author.username} tickles ${person.username} \n\n`,
        embeds: [embed],
